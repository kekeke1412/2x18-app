...
import {
  ...
  CheckCircle2, Activity, Edit3, Check, X, GraduationCap, BarChart2, Hexagon, Sparkles, Loader2, ArrowRight
} from 'lucide-react';
...
function SemesterSelector({ semGPA, semesterNames, updateSemesterName }) {
  ...
  // Fixed scale for Y axis from 0 to 4
  const chartMin = 0.0;
  const chartMax = 4.0;
  const range = chartMax - chartMin;

  const getX = (i) => ((i + 0.5) / N) * 100;
  // Map GPA 0->4 to Y 90%->10% of the SVG height
  const getY = (gpa) => 90 - (parseFloat(gpa) / range) * 80;
