...
import { useSmeMap, useDocs, useSubjectTasks, useSubjectComments, useMembers } from '../hooks/useDomainQueries';
...
function SubjectTaskPanel({ subjectId, isSme, currentUser, onOpenStats }) {
  const { addSubjectTask, editSubjectTask, deleteSubjectTask, tickSubjectTask } = useApp();
  const { data: subjectTasks = {} } = useSubjectTasks();
  const tasks = toArr(subjectTasks[subjectId]);
...
function SubjectCard({ sub, grade, sme, isCore, isSme, onChangeSme, onUpload, docs: docsProp, members, currentUser, rateDoc, grades, onViewProgress, onOpenStats }) {
  const { addSubjectComment, deleteDoc } = useApp();
  const { data: subjectTasks = {} } = useSubjectTasks();
  const { data: subjectComments = {} } = useSubjectComments();
...
export default function Subjects() {
  const { currentUser, isCore, setSme, addDoc, rateDoc, grades, myGrades } = useApp();
