...
import { useMembers, useContributions, useGamifTitles, useGamifAwards, useGamifSeasons } from '../hooks/useDomainQueries';

/* ═══════════════════════════════════════════════════════════════
   HỆ THỐNG RANK — 20 bậc (Đồng I→III … Tinh Anh I→III, Huyền Thoại, Cao Thủ)
═══════════════════════════════════════════════════════════════ */
const RANKS = [
  { id:'d1', tier:'Đồng',       sub:'I',   min:0,           max:14_999,      color:'#b87333', glow:'rgba(184,115,51,0.55)', bg:'rgba(184,115,51,0.09)' },
  { id:'d2', tier:'Đồng',       sub:'II',  min:15_000,      max:29_999,      color:'#c08840', glow:'rgba(192,136,64,0.55)', bg:'rgba(192,136,64,0.09)' },
  { id:'d3', tier:'Đồng',       sub:'III', min:30_000,      max:49_999,      color:'#d09050', glow:'rgba(208,144,80,0.55)', bg:'rgba(208,144,80,0.09)' },
  { id:'s1', tier:'Bạc',        sub:'I',   min:50_000,      max:79_999,      color:'#9ca3af', glow:'rgba(156,163,175,0.55)', bg:'rgba(156,163,175,0.08)' },
  { id:'s2', tier:'Bạc',        sub:'II',  min:80_000,      max:109_999,     color:'#b0b7c0', glow:'rgba(176,183,192,0.55)', bg:'rgba(176,183,192,0.08)' },
  { id:'s3', tier:'Bạc',        sub:'III', min:110_000,     max:149_999,     color:'#c8cfd8', glow:'rgba(200,207,216,0.55)', bg:'rgba(200,207,216,0.08)' },
  { id:'g1', tier:'Vàng',       sub:'I',   min:150_000,     max:229_999,     color:'#fbbf24', glow:'rgba(251,191,36,0.55)',  bg:'rgba(251,191,36,0.08)' },
  { id:'g2', tier:'Vàng',       sub:'II',  min:230_000,     max:309_999,     color:'#fcd34d', glow:'rgba(252,211,77,0.55)',  bg:'rgba(252,211,77,0.08)' },
  { id:'g3', tier:'Vàng',       sub:'III', min:310_000,     max:399_999,     color:'#fde68a', glow:'rgba(253,230,138,0.55)', bg:'rgba(253,230,138,0.08)' },
  { id:'p1', tier:'Bạch Kim',   sub:'I',   min:400_000,     max:529_999,     color:'#bfdbfe', glow:'rgba(191,219,254,0.5)',  bg:'rgba(191,219,254,0.06)' },
  { id:'p2', tier:'Bạch Kim',   sub:'II',  min:530_000,     max:659_999,     color:'#c8e8ff', glow:'rgba(200,232,255,0.5)',  bg:'rgba(200,232,255,0.06)' },
  { id:'p3', tier:'Bạch Kim',   sub:'III', min:660_000,     max:799_999,     color:'#ddf0ff', glow:'rgba(221,240,255,0.5)',  bg:'rgba(221,240,255,0.06)' },
  { id:'k1', tier:'Kim Cương',  sub:'I',   min:800_000,     max:999_999,     color:'#38bdf8', glow:'rgba(56,189,248,0.6)',   bg:'rgba(56,189,248,0.08)' },
  { id:'k2', tier:'Kim Cương',  sub:'II',  min:1_000_000,   max:1_199_999,   color:'#0ea5e9', glow:'rgba(14,165,233,0.6)',   bg:'rgba(14,165,233,0.08)' },
  { id:'k3', tier:'Kim Cương',  sub:'III', min:1_200_000,   max:1_499_999,   color:'#7dd3fc', glow:'rgba(125,211,252,0.6)',  bg:'rgba(125,211,252,0.09)' },
  { id:'e1', tier:'Tinh Anh',   sub:'I',   min:1_500_000,   max:1_999_999,   color:'#c084fc', glow:'rgba(192,132,252,0.6)',  bg:'rgba(192,132,252,0.09)' },
  { id:'e2', tier:'Tinh Anh',   sub:'II',  min:2_000_000,   max:2_499_999,   color:'#a855f7', glow:'rgba(168,85,247,0.6)',   bg:'rgba(168,85,247,0.09)' },
  { id:'e3', tier:'Tinh Anh',   sub:'III', min:2_500_000,   max:2_999_999,   color:'#d946ef', glow:'rgba(217,70,239,0.6)',   bg:'rgba(217,70,239,0.09)' },
  { id:'hl', tier:'Huyền Thoại',sub:null,  min:3_000_000,   max:4_999_999,   color:'#fb923c', glow:'rgba(251,146,60,0.65)',  bg:'rgba(251,146,60,0.1)'  },
  { id:'ct', tier:'Cao Thủ',    sub:null,  min:5_000_000,   max:Infinity,    color:'#fde047', glow:'rgba(253,224,71,0.7)',   bg:'rgba(253,224,71,0.1)'  },
];
...
function Avatar({ m, size, isMe }: any) {
  return <UserAvatar user={m} size={size} isMe={isMe} />;
}

/* ── Leaderboard row ── */
function LbRow({ m, i, currentUser, onRevoke }: any) {
...
export default function Gamification() {
  const { currentUser, isSuperAdmin, isCore } = useApp();
  const { data: members = [] } = useMembers();
  const { data: contributions = {} } = useContributions();
  const { data: titles = [] } = useGamifTitles();
  const { data: awards = {} } = useGamifAwards();
  const { data: seasons = [] } = useGamifSeasons();
  ...
  /* ──────────────────── INPUT STYLE ──────────────────── */
  const inputSt = {
    width:'100%', padding:'8px 11px', borderRadius:8, boxSizing: 'border-box' as any,
        ...
        {tab === 'howto' && (
          <div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {[
                // ── Cộng điểm
                ['+1,000', '📄','Upload tài liệu môn học'],
                ['+1,000', '📊','Đăng báo cáo tổng kết'],
                ['+500',   '📚','Tạo học phần Vocabulary mới'],
                ['+1,000', '💯','Hoàn thành Quiz đạt 100%'],
                ['+500',   '🧠','Đạt cấp độ "Nhớ sâu" (Lv 6) cho 1 từ'],
                ['+1,000', '⭐','Tài liệu được đánh giá 5 sao'],
                ['+500',   '✅','Điểm danh buổi họp SME (1 lần/buổi)'],
                ['+500',   '⏰','Hoàn thành task đúng hạn'],
                ['+1,000', '📈','Tiến độ môn học đạt 100%'],
                ['+200',   '🗳️','Tham gia bình chọn nhóm'],
