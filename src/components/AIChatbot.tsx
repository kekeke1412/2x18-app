...
import { useTasks, useCalEvents, useAttendance, useContributions, useVocab, useUserVocab, useDocs } from '../hooks/useDomainQueries';
...
export default function AIChatbot() {
  const { currentUser, myGrades, myGradesEnriched } = useApp();
  const { data: tasks = [] } = useTasks();
  const myTasks = tasks.filter(t => t.userId === currentUser?.id || t.assignees?.includes(currentUser?.id));
